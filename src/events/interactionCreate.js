              ...
              for (const panel of panels) {
                ...
                if (!channel) {
                  await deleteReactionRoleMessage(client, guildId, panel.messageId).catch((err) => {
                    logger.warn(`Failed to clean up orphaned reaction role panel ${panel.messageId}:`, err.message);
                  });
                  continue;
                }
              ...
              const choices = await Promise.all(
                validPanels.slice(0, 25).map(async panel => {
                  try {
                    ...
                    logger.debug('Error fetching reaction role panel for autocomplete:', e.message);
                    return null;
                  }
                })
              );
